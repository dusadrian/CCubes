...
There is a minimal help system integrated into CCubes. `-d` canonicalizes PI ordering for deterministic experiments. Users can enable diagnostic logging with `-dbg<level>`; this provides insights into the internal workings of the minimization process and can be useful for troubleshooting. The debug levels are preliminary, and more detailed logging functionality will be added in the future.
